    ...
    const fetchWeather = async () => {
      try {
        const apiRes = await fetch(
          ...
          }/api/fetchWeather?lat=${selectedCity.lat}&lon=${selectedCity.lng}`
        ).then((res) => res.json());
  ...
  return (
    <div className="test2-blur-overlay">
        ...
        {apidata.current_condition ? (
          <button
            onClick={() => {
              ...
              setweatherData({
                c_code: selectedCity.countryCode,
                clicked: false,
                visible: false,
                temp: apidata.current_condition[0].temp_C + "°C",
                pressure: apidata.current_condition[0].pressure,
                humidity: apidata.current_condition[0].humidity,
                windSpeed: apidata.current_condition[0].windspeedKmph,
                uvIndex: apidata.current_condition[0].uvIndex,
                weatherDescription:
                  apidata.current_condition[0].weatherDesc[0].value,
                feels_Like: apidata.current_condition[0].FeelsLikeC,
                forecastDates: paginateForecast(apidata.weather),
              });
            }}
          >
            SEARCH
          ...
          <div style={{ margin: "10px auto" }}>
            <ClipLoader color={"#38c7b2"} loading={true} size={"10px"} />
          </div>
        )}
      </div>
      ...
      <div className="location-search-close-btn">
        <i
          onClick={() => setweatherData({ clicked: false })}
